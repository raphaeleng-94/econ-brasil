...
""")
print(f"latest: {cur.rowcount}", flush=True)
...
""")
print(f"monthly: {cur.rowcount}", flush=True)
...
""")
print(f"annual: {cur.rowcount}", flush=True)

cur.execute("INSERT INTO bronze.pipeline_log(layer,status,records_loaded,executed_at) VALUES('gold','success',0,NOW())")
