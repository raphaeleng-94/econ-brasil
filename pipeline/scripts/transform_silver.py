"""Silver Layer - Limpeza e tipagem"""
...
def run():
    ...
    """, batch)
    cur.execute("INSERT INTO bronze.pipeline_log(layer,status,records_loaded,executed_at) VALUES('silver','success',%s,NOW())",(len(batch),))
    conn.commit(); conn.close()
    logger.success(f"Silver: {len(batch)} processados, {skipped} ignorados")
