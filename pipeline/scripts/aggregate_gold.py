...
print("Building gold.latest_indicators...", flush=True)
cur.execute("""
...
""")
print(f"  latest: {cur.rowcount} rows", flush=True)

print("Building gold.monthly_summary...", flush=True)
cur.execute("""
...
""")
print(f"  monthly: {cur.rowcount} rows", flush=True)

print("Building gold.annual_summary...", flush=True)
cur.execute("""
...
""")
print(f"  annual: {cur.rowcount} rows", flush=True)

cur.execute(
    "INSERT INTO bronze.pipeline_log(layer,status,records_loaded,executed_at) "
    "VALUES('gold','success',%s,NOW())", (cur.rowcount,)
)
